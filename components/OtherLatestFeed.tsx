...
const OtherLatestFeed = () => {
  const [latestBlogs, setLatestBlogs] = useState<otherBlog[]>([]);
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {isLoading ? (
      <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-72 sm:h-80 backdrop-blur-sm bg-black/5 border border-black/10 rounded-2xl animate-pulse shadow-lg" />
        ))}
      </div>
    ) : (
      <motion.div
        ...
        {latestPDFs.map((pdf) => (
          <motion.div key={pdf._id} variants={scaleIn}>
            <Link href={`/otherChannel/pdf/${pdf._id}`} className="block group">
              <article className="h-full backdrop-blur-lg bg-white/90 border border-gray-200 rounded-2xl overflow-hidden transition-all duration-500 group-hover:bg-white group-hover:border-gray-300 group-hover:scale-105 group-hover:shadow-2xl shadow-lg flex flex-col">
                {pdf.image ? (
                  <div className="relative w-full h-40 sm:h-44 lg:h-48 overflow-hidden flex-shrink-0">
                    <Image
                      src={pdf.image}
                      alt={pdf.title}
                      fill
                    ...
                    {/* PDF indicator */}
                    <div className="absolute top-3 sm:top-4 right-3 sm:right-4 backdrop-blur-sm border border-gray-400/50 rounded-full p-2 shadow-md">
                      <Download className="size-4 sm:size-5 text-white" />
                    </div>
                  </div>
                ) : (
                  <div className="w-full h-40 sm:h-44 lg:h-48 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center flex-shrink-0" />
                )}

                <div className="p-4 sm:p-6 flex flex-col flex-grow">
                  <h3 className="text-base sm:text-lg lg:text-xl font-bold text-gray-900 text-right line-clamp-2 font-faseyha mb-2 sm:mb-3 group-hover:text-gray-800 transition-colors duration-300">
                    {pdf.title}
                  </h3>
                  <p className="text-gray-600 text-xs sm:text-sm lg:text-base text-right line-clamp-2 sm:line-clamp-3 font-faseyha leading-relaxed flex-grow">
...
export default OtherLatestFeed;