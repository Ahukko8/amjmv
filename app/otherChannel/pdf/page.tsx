...
export default function OtherPDFsPage() {
  const [pdfs, setPdfs] = useState<OtherPDF[]>([]);
  ...
  const fetchPDFs = async () => {
    try {
      const response = await fetch(`/api/otherPdfs?page=${currentPage}&limit=${ITEMS_PER_PAGE}`);
              ...
              >
                {pdfs.map((pdf) => (
                  <motion.div key={pdf._id} variants={fadeInUp}>
                    <Link href={`/otherChannel/pdf/${pdf._id}`} className="block group h-full">
